...
contract('NodeControlSimple', (accounts) => {
    ...
    describe('Functional requirement tests', () => {
        ...
        it('must emit UpdateAvailable event when a new update is triggered on a specific validator', async () => {
            await nodeControlSimple.updateValidator(accounts[1], '0x01', "dockerName123", '0x02', "chainSpecUrl123", true, {
                from: accounts[0]
            });
        ...
        it('should emit multiple UpdateAvailable events when more then one validator is triggered for update', async () => {
            await nodeControlSimple.updateValidator(accounts[2], '0x01', "dockerName123", '0x02', "chainSpecUrl123", true, {
                from: accounts[0]
            });
            await nodeControlSimple.updateValidator(accounts[3], '0x03', "dockerName123", '0x04', "chainSpecUrl123", true, {
                from: accounts[0]
            });
